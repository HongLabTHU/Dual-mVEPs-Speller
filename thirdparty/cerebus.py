...
def getdigfactor(ext_headers, idx):
    ...
    return float(max_analog - min_analog) / float(max_digital - min_digital) * 1e-3
...
class NevFile:
    ...
    def __init__(self, datafile=''):
        ...
        # open the file passed or allow user to browse to one
        self.datafile = open(self.datafile, 'rb')
...
class NsxFile:
    ...
    def getdata(self, elec_ids='all', start_time_s=0, data_time_s='all', downsample=1):
        ...
        try:
            output['data'] = np.zeros((total_samps, num_elecs), dtype=np.float64)
        ...
        while not hit_stop:
            ...
            for loop in range(num_loops):
                ...
                if elec_id_indices:
                    output['data'][d_ptr:d_ptr + mm.shape[0]] = np.array(mm[:, elec_id_indices]).astype(np.float64)
                else:
                    output['data'][d_ptr:d_ptr + mm.shape[0]] = np.array(mm).astype(np.float64)
                d_ptr += mm.shape[0]
                del mm
